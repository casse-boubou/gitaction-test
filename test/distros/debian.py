# debian.py - Fonctions available for an debian docker image
import gzip
import json
import lzma
import os
...
import requests
from scripts.enum_class import LatestSource, DebianRelease
...
def catch_version(version):
    ...
    if version == "latest":
        version = str(LatestSource.search_value("debian"))
    version_match = re.search(
        r"(?P<major>[0-9]+).?(?:.*)",
        version)
    if version_match:
        version = int(version_match.group('major'))
    return DebianRelease.search_key(version).lower()
...
def download_file_with_progress(url, save_path, debian_version, pocket, component, chunk_size=1024):
    ...
    with requests.get(url, stream=True, timeout=20) as response:
        ...
        desc = f"Downloading debian {debian_version} {pocket} {component}"
        desc = f"{desc:<50.50}"
        progress_bar = tqdm(
            desc=desc,
            total=total_size,
...
def download_fresh_depot(distribution, version):
    """Download a fresh version of debian depot"""
    pockets = ["base", "updates", "security", "backports"]
    components = ["main", "contrib", "non-free", "non-free-firmware"]

    # Catch debian version
    debian_version = catch_version(version)

    # Create Directory for version/pocket
    for pocket in pockets:
        directory_name = define_path(distribution=distribution, version=debian_version, repository=pocket)[0]
        try:
    ...
    for pocket in pockets:
        ...
        for component in components:
            if pocket == "base":
                url = f"http://deb.debian.org/debian/dists/{debian_version}/{component}/binary-amd64/Packages.{packages_ext}"
                urls[pocket][component] = url
            elif pocket == "security":
                url = f"https://security.debian.org/debian-security/dists/{debian_version}-{pocket}/{component}/binary-amd64/Packages.{packages_ext}"
                urls[pocket][component] = url
            else:
                url = f"http://deb.debian.org/debian/dists/{debian_version}-{pocket}/{component}/binary-amd64/Packages.{packages_ext}"
                urls[pocket][component] = url

    # Download and extract archive if not exist
    for pocket, components in urls.items():
        for component, url in components.items():
            srcfilepath = define_path(distribution, debian_version, pocket, component, packages_ext)[1]
            destfilepath = define_path(distribution, debian_version, pocket, component)[2]
            fiile = Path(srcfilepath)
            if not fiile.exists():

                # Download archive
                download_file_with_progress(url, srcfilepath, debian_version, pocket, component)
...
def compare(distribution, version, packages):
    ...
    for pocket in pockets:
        for component in components:
            filefolder = define_path(distribution, debian_version, pocket)[3]
